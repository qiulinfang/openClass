    ...
    >
      ...
      >
        ...
        <view class="chat-panel-header">
          ...
          <!-- 形态切换按钮：panel <-> dialog -->
          <BaseButton variant="ghost" size="sm" class="toggle-mode-button" @click="emit('toggle-mode')">
            <image :src="switcherIcon" mode="aspectFit" class="toggle-mode-icon" />
          </BaseButton>
          <!-- WebSocket连接状态 -->
          <view v-if="isDev" class="connection-status" @click="logWebSocketStatus">
            <view
          ...
          <BaseButton
            ...
            class="close-button"
          >
            <image src="/icons/close.svg" style="width: 24px; height: 24px;" />
          </BaseButton>
        </view>
            ...
            <view class="main-chat-body">
              ...
              <view class="right-panel">
                ...
                <view v-else class="empty-chat">
                  <image src="/icons/chat.svg" style="width: 48px; height: 48px;" class="q-icon-fallback" />
                  <view class="empty-text">请选择或创建一个会话</view>
                </view>
              </view>
            </view>
          </view>
...
const handleOpenHtmlPreview = (payload: { url: string; html?: string }) => {
  ...
  // 获取当前会话 ID
  const currentSessionId = aiGeneralStore.currentSession?.sessionId
  console.log('[MainChatPanel] handleOpenHtmlPreview:', { url, hasHtml: !!html, currentSessionId })
