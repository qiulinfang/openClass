<template>
  <view class="chat-panel-container">
    <!-- 探索遮罩（在选择探索/截图工具时显示） -->
    <view v-if="isExploring" class="explore-overlay" @click.stop>
      <image :src="textbookipIcon" mode="aspectFit" class="explore-icon textbookip" />
      <image :src="ipWordIcon" mode="aspectFit" class="explore-icon ipWord" />
    </view>
    <!-- 遮罩层上的按钮（独立于遮罩层，在小程序中建议通过 view 定位，Teleport 支持有限） -->
    <view
    ...
    >
      <image :src="selectAndAskIconToUse" mode="aspectFit" style="width: 100%; height: 100%" />
    </view>
        ...
        <view class="session-card-wrapper">
          ...
          >
            ...
            <template #card-body="{ card }">
              <view class="chat-snapshot" @click="handleSessionCardClick(card.id)">
                <view class="snapshot-messages">
                  <template
                  ...
                  >
                    ...
                    >
                      <rich-text
                        ...
                        :nodes="renderMessageContent(content)"
                      ></rich-text>
                    </view>
                  </template>
                  <view v-else class="snapshot-empty">
                    <text>点击开始对话</text>
                  </view>
                </view>
                <view class="snapshot-fade"></view>
              </view>
            </template>
          </CardStack>
        </view>
        <!-- 底部操作条 -->
        <view class="session-bottom-bar">
          <Button
            label="返回"
    ...
    >
      <text>确定要清除当前题目的所有会话吗？此操作不可撤销。</text>
    </Dialog>
...
const handleOpenHtmlPreview = (payload: { url: string; html?: string }) => {
  ...
  // 如果有缓存的 HTML，存入 uni.setStorageSync 供 HtmlPreviewView 使用
  if (html) {
    uni.setStorageSync('htmlPreview_inlineContent', html)
  }
...
const updateOverlayButtonPosition = async () => {
  ...
  try {
    ...
    query.select('.chat-content-container .tab-content .toolbar-btn').boundingClientRect()
    
    query.exec((res) => {
      if (!res || !res[0]) {
        console.warn('[ExerciseChatPanelNew] 找不到实际按钮元素位置')
        return
      }
      
      const triggerRect = res[0]
      
      // 更新遮罩层按钮样式 - 使用固定定位，大小与实际按钮一致
