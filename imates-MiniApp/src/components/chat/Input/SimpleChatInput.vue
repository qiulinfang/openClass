<template>
  <view>
    ...
    <view class="simple-chat-top-slot">
      <slot name="top"></slot>
    </view>
    ...
    <view class="simple-chat-input-wrapper">
      <!-- 模式选择器（与 ChatInput.vue 相同实现） -->
      <BubblePopup v-model="showModeSelectorMenu" placement="top" :offset="8">
        <template #trigger>
          <button class="action-mode-btn" :class="{ active: true }" @click="toggleModeSelector">
            <image :src="getModelIcon(props.selectedModel)" mode="aspectFit"
      ...
      <view class="input-area">
        <input :value="localInputValue" class="simple-chat-input" type="text" :placeholder="placeholder"
          @confirm="handleEnter" @focus="handleFocus" @blur="handleBlur" @input="(e) => localInputValue = e.detail.value" />
      </view>

      <view class="right-actions">
        <button class="simple-send-btn" :disabled="!localInputValue.trim() || isLoading"
          @click="handleSend">
