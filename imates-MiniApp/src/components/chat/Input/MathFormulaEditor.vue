<template>
  <view class="math-formula-editor">
    <!-- 完整模式 -->
    <view class="container">
      <view class="main-content">
        ...
        <view class="editor-section">
          ...
          <!-- Quill编辑器 (在小程序中通常需要用原生 editor 组件或简单的 textarea) -->
          <view
            ref="editorRef"
            :id="editorId"
            class="quill-editor"
...
const initializeEditor = async () => {
  try {
    ...
    // 监听键盘事件
    // 在小程序中建议通过组件的 @confirm 等事件处理，此处保留逻辑但注意兼容性
    const query = uni.createSelectorQuery().in(instance)
    query.select(`#${editorId.value}`).boundingClientRect()
    query.exec((res) => {
      // 小程序中通常不支持直接对获取到的节点 addEventListener
      console.log('Quill editor container res', res)
    })
