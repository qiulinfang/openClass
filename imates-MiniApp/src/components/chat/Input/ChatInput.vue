<template>
  <view class="modern-chat-container">
    ...
    <view v-if="showToolbar" class="chat-input-header-flex">
      <!-- 顶层：允许整块头部完全自定义 -->
      <slot name="header-all">
        ...
        <view class="chat-input-header-left">
          <view class="chat-top-toolbar">
            ...
            >
              <image :src="picturIcon" mode="aspectFit" class="toolbar-icon" />
            </button>
            <!-- 前置插槽：保留向后兼容 -->
      ...
      <view v-if="props.quotedMessage" class="quote-bar">
        <view class="quote-content">
          <view class="quote-text">
            <text class="quote-message">{{
              truncateQuoteContent(props.quotedMessage.content)
            }}</text>
          </view>
        </view>
        <!-- 关闭引用按钮 -->
        <button class="quote-close" @click.stop="emit('remove-quote')">
          <text class="close-icon">✕</text>
        </button>
      </view>
      <!-- 截图缩略图（挂在输入框内左上角），支持多张 -->
      <view v-if="screenshotsToShow.length" class="screenshot-thumb-bar">
        <view
      ...
      <view class="control-bar">
        ...
        <view class="left-controls">
          ...
          <view v-if="props.isEditing" class="edit-indicator">
            ...
            <button @click.stop="handleCancelEdit" class="cancel-edit-btn">
              <text class="close-icon">✕</text>
            </button>
          </view>
          ...
          <template v-if="!props.isEditing">
            ...
            >
              <template #trigger>
                ...
                >
                  ...
                  />
                  <text>{{ getModelDisplayName(props.selectedModel) }}</text>
                </button>
              </template>
            ...
            <button
              ...
              "
            >
              <image :src="onlineSearchIconToUse" mode="aspectFit" class="toolbar-icon" />
            </button>
          ...
          >
            <image :src="isAskTeacherSelected ? askTeacherIconSelected : askTeacherIcon" mode="aspectFit" class="toolbar-icon" />
          </button>
          </template>
        </view>
        ...
        <view class="right-controls">
          ...
          <view class="send-button-hit-area" @click="handleSendButtonHitAreaClick">
            ...
            >
              ...
              />
              <!-- 编辑状态图标 -->
              <image v-else-if="props.isEditing" :src="sendIcon" mode="aspectFit" class="send-icon" />
              <!-- 自定义发送图标 -->
              <image v-else :src="sendIcon" mode="aspectFit" class="send-icon" />
            </button>
          </view>
        </view>
      </view>
