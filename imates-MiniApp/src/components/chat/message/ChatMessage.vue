<template>
  <!-- 时间分隔符消息 -->
  <view v-if="message.messageType === 'time_separator'" class="time-separator">
    <text class="time-separator-text">{{ formatTimeSeparator(message.timestamp) }}</text>
  </view>
  ...
  >
    <!-- 选择模式下的复选框 -->
    <view v-if="isSelectionMode" class="message-checkbox" @click.stop>
      <Checkbox
    ...
    <!-- AI/老师消息 -->
    <view v-if="message.sender !== 'user'" class="ai-message">
      <view class="ai-avatar">
        <image :src="aiAvatarIcon" mode="aspectFit" class="avatar-img" />
      </view>
      <view class="ai-content">
        ...
        <view class="ai-content-container">
          ...
          >
            <template #trigger>
              <view class="ai-message-content" :ref="(el) => setBubbleRef(el, 'ai')">
                <!-- 语音消息 -->
                <VoiceMessage
                ...
                <template
                  ...
                  "
                >
                  <view v-paste-to-draft="{ onPaste: handlePasteToDraft, enabled: isPasteToDraftEnabled }">
                    <!-- 显示图片 -->
                    <ImageMessage
                ...
                >
                  ...
                  >
                    <view class="error-message">
                      <rich-text :nodes="renderedContent" @click="handleImageClick"></rich-text>
                      <view v-if="message.retryCount && message.retryCount > 0" class="retry-count">
        ...
        <view v-if="actionButtons.length > 0" class="message-actions">
          <button
          ...
          >
            <image v-if="button.iconPath" :src="button.iconPath" mode="aspectFit" class="action-icon" />
            <image
    ...
    <view v-else class="user-message">
      <view class="user-content">
        <view class="user-bubble-row">
          <!-- 已读状态标签 -->
          <view v-if="showReadStatus && message.sender === 'user'" class="read-status">
            <text class="read-status-text">{{ message.isRead ? '已读' : '未读' }}</text>
          </view>
          <!-- 用户气泡 -->
          <BubblePopup
            v-model="showActionMenu"
          ...
          >
            <!-- 引用消息区域 -->
            <template #trigger>
              <view class="user-bubble" :ref="(el) => setBubbleRef(el, 'user')">
                <!-- 语音消息 -->
                <VoiceMessage
                  v-if="message.messageType === 'voice' && message.voiceData"
                  :file-path="message.voiceData.filePath"
...
const bubbleTarget = ref<any>(null)
...
const handleCopy = async () => {
  try {
    ...
    if (props.message.messageType === 'voice') {
      ...
      // 小程序中无法使用 document.createElement 剥离 HTML，采用正则简化处理
...
// 设置气泡引用
const setBubbleRef = (el: any, type: 'ai' | 'user') => {
  if (el) {
...
const ensureBubbleTarget = (eventTarget: any) => {
  // 如果气泡引用已经存在，直接返回
  if (bubbleTarget.value) {
    return bubbleTarget.value
  }

  // 小程序中不支持 closest，建议通过 ref 直接绑定，这里改为返回 null
  return null
}

// 动态计算气泡框位置
const calculateBubblePosition = (eventTarget?: any) => {
  // 如果没有气泡引用，尝试从事件目标中获取
  if (!bubbleTarget.value && eventTarget) {
    ensureBubbleTarget(eventTarget)
  }

  // 小程序中定位逻辑已交由组件内部实现，此处仅保留占位
}
...
const setStreamingRef = (el: any) => {
  if (el) {
    // 对于流式消息，使用懒加载模式
    nextTick(() => {
      // 在小程序中 MathJax 渲染逻辑需要重构为非 DOM 操作方式，此处仅保留占位
    })
  }
}

const setStaticRef = (el: any) => {
  if (el) {
    // 对于静态消息，使用懒加载模式
    nextTick(() => {
      // 在小程序中 MathJax 渲染逻辑需要重构为非 DOM 操作方式，此处仅保留占位
