<template>
  <view class="session-list">
    <!-- 顶部标题栏（非批量选择模式） -->
    <view v-if="!isSelectionMode && showHeader" class="session-header">
    ...
    <!-- 搜索栏 -->
    <view v-if="!isSelectionMode && showHeader" class="search-bar">
    ...
    <!-- 会话列表 -->
    <RubberBandList ref="rubberBandListRef" v-if="filteredRecords && filteredRecords.length > 0" class="scroll-wrapper">
      <view class="scroll-content">
        <view ref="sessionItemsRef" class="session-items">
          <!-- 置顶会话区域 -->
          <view v-if="pinnedRecords.length > 0" class="date-group-wrapper">
            <view class="date-group-header">置顶</view>
            <!-- 会话列表 -->
            <SessionItem
          ...
          <!--  按日期分组 -->
          <view v-for="(group, date) in groupedRecords" :key="date" class="date-group-wrapper">
            <view class="date-group-header">{{ date }}</view>
            <!-- 会话列表 -->
            <SessionItem
    ...
    <view v-if="isSelectionMode" class="selection-toolbar">
      ...
      <view class="selection-left">
        ...
        <text class="selection-count">已选{{ selectedRecords.size }}/{{ filteredRecords.length }}</text>
      </view>
    ...
    >
      <text>确定要删除选中的 {{ selectedRecords.size }} 个会话吗？删除后无法恢复。</text>
    </Dialog>
  </view>
</template>
...
import type { AiTextbookSession } from '@/types/index.js'
...
// ==================== DOM 引用 ====================
const sessionItemsRef = ref<any>(null)
const rubberBandListRef = ref<any>(null)

// 流程：滚动到列表顶部
const scrollToTop = () => {
  if (rubberBandListRef.value) {
    rubberBandListRef.value.scrollToTop()
  }
}
