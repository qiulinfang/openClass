<template>
  ...
  <view class="session-tree">
    <!-- ============ 顶部搜索栏区域 ============ -->
    <view class="tree-header">
      <!-- 搜索输入框 - 支持实时搜索会话 -->
      <SearchInput :modelValue="searchKeyword" @update:modelValue="searchKeyword = $event" placeholder="搜索会话" class="search-input-custom">
        <template #append>
          <image :src="search1Icon" mode="aspectFit" class="search-icon" />
        </template>
      </SearchInput>
    ...
    <!-- ============ 会话列表区域 ============ -->
    <!-- 仅在有会话数据时显示列表 -->
    <view v-if="treeNodes.length > 0" ref="scrollWrapper" class="scroll-wrapper">
      ...
      >
        ...
        <view class="accordion-list">
          <!-- ============ 分类项循环 ============ -->
          <view v-for="category in treeNodes" :key="category.id" class="accordion-item">
            <!-- ============ 分类头部 - 点击展开/收起 ============ -->
            <view class="accordion-header" @click="toggleCategory(category.id)">
              ...
              <view class="category-actions">
                ...
                >
                  <image :src="addSessionIcon" mode="aspectFit" class="create-session-icon" />
                </view>
            ...
            <!-- ============ 会话列表 - 带过渡动画 ============ -->
            <view v-show="expandedCategories.has(category.id)" class="accordion-body">
              <!-- ============ 会话项循环 ============ -->
              <view
              ...
              >
                ...
                <view class="session-content">
                  ...
                  <view class="session-main">
                    <!-- 会话标题：AI会话名称或教师科目名称 -->
                    <text class="session-title">{{ session.label }}</text>

                    <!-- 未读消息红点 - 有未读消息时显示 -->
                    <view v-if="hasUnreadMessage(session)" class="unread-badge"></view>
                  </view>

                  <!-- 会话副标题 - 可选显示 -->
                  <view v-if="session.subtitle" class="session-subtitle">
                    {{ session.subtitle }}
                  </view>
                </view>
                ...
                >
                  <!-- 触发按钮 - 更多操作按钮 -->
                  <template #trigger>
                    <BaseButton variant="ghost" size="sm" class="more-btn" label="..." />
                  </template>

                  <!-- ============ 菜单内容 ============ -->
                  <view class="session-more-menu-card">
                    <!-- AI会话专用操作 -->
                    <template v-if="session.category === 'ai'">
                      ...
                      >
                        ...
                        />
                        <view>{{ session.pinned ? '取消置顶' : '置顶' }}</view>
                      </view>
                      ...
                      >
                        <image
                          ...
                          style="width: 18px; height: 18px"
                        />
                        <view>{{ session.favorited ? '取消收藏' : '收藏' }}</view>
                      </view>

                      <!-- 分隔线 -->
                      <view class="session-more-menu-divider"></view>
                    </template>

                    <!-- 通用操作：删除会话 -->
                    <view class="session-more-menu-delete-wrapper">
                      <view
                        class="more-menu-item-row"
                        @click="
                        ...
                        "
                      >
                        <image :src="deleteIcon" mode="aspectFit" style="width: 18px; height: 18px" />
                        <view class="text-delete">删除</view>
                      </view>
                    </view>
                  </view>
                </BubblePopup>
    ...
    <view v-else class="empty-state">
      ...
      <view class="q-mt-md text-h6 text-grey-6">
        {{ searchKeyword ? '未找到匹配的会话' : '暂无会话' }}
      </view>

      <!-- 空状态描述 - 提供相应的提示信息 -->
      <view class="q-mt-sm text-caption text-grey-5">
    ...
    >
      <!-- 显示具体的删除确认内容 -->
      <text>{{ deleteConfirmContent }}</text>
    </Dialog>
  </view>
</template>
