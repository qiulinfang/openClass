<template>
  <!-- 聊天视图主容器 - 支持键盘动画状态 -->
  <view ref="chatViewRef" class="chat-view" :class="{ 'keyboard-animating': isKeyboardAnimating }">
    <!-- 聊天消息区域 - 占据全宽度，支持滚动 -->
    <view class="chat-messages-container">
      ...
      >
        ...
        <!-- 顶部加载指示器（教师聊天分页加载） -->
        <view v-if="type === 'teacher' && teacherStore.pagination.isLoadingMore"
        ...
        <!-- 没有更多数据提示（教师聊天） -->
        <view v-if="type === 'teacher' && !teacherStore.pagination.hasMore && displayedMessages.length > 0"
      ...
      >
        <view class="new-message-icon"></view>
      </button>
      <!-- 会话列表覆盖层：使用 CardStack 展示多会话管理界面（仅覆盖消息区域） -->
      <view
        v-if="showSessionListPanel"
      ...
      >
        ...
        >
          ...
          <template #card-body="{ card }">
            <view class="chat-snapshot" @click="handleSwitchSession(card.id)">
    ...
    <view v-if="isSelectionMode" class="selection-toolbar">
      ...
      <view class="selection-left">
        ...
        <text class="selection-count">已选{{ selectedMessages.size }}/{{ displayedMessages.length }}</text>
      </view>
    ...
    <!-- 在多选模式下隐藏输入组件 -->
    <view v-if="!isSelectionMode" class="chat-input-area">
      ...
      <!-- 底部提示文案：移动到输入区域内部底部 -->
      <view v-if="showFooterText" class="chat-footer-text">
        与学伴共学，敢质疑、会判断，思维不设限!
      </view>
    </view>
