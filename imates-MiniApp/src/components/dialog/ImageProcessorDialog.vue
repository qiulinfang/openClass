<template>
  ...
  >
    <template #header-right>
      <BaseButton variant="ghost" size="sm" @click="handleDebugLog">
        <image src="/icons/bug_report.svg" style="width: 20px; height: 20px;" />
      </BaseButton>
    </template>
    <view class="screenshot-input-body">
      <!-- 左侧：截图编辑区域（集成 drawingBoardNew） -->
      <view class="screenshot-editor">
        <drawingBoardNew
          v-if="previewImage"
          key="drawing-mode"
          ref="drawingBoardRef"
        ...
        <view v-else class="empty-placeholder">
          <image src="/icons/image.svg" style="width: 48px; height: 48px;" class="q-icon-fallback" />
          <text>暂无截图</text>
        </view>
      </view>
      <!-- 右侧：截图缩略图列表 + 继续截图按钮（竖直排列，仅多截图模式下显示） -->
      <view v-if="props.mode === 'multiple'" class="screenshot-side-panel">
        <view class="side-panel-body">
          <view v-if="thumbnailList.length" class="side-thumbs-list">
            <ScreenshotThumb
          ...
          <view v-else class="side-empty-text">暂无可用截图</view>
        </view>
        <view class="side-panel-footer" v-if="thumbnailList.length < MAX_SCREENSHOTS">
          <BaseButton
            variant="outline"
            class="add-more-btn"
