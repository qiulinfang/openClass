<template>
  ...
  >
    <view v-show="!props.screenshotFlowVisible" class="global-chat-content">
      <!-- 左侧聊天记录（树形结构） -->
      ...
      <view class="right-panel">
        ...
        >
          ...
          <template #header-right>
            <view class="header-right-actions">
              <view @click="handleNewChatClick" class="add-session-btn">
                <image :src="addSessionIcon" mode="aspectFit" class="add-session-icon" />
              </view>
            </view>
          </template>
        </ChatView>
        ...
        <view v-else class="empty-chat">
          <image src="/icons/chat.svg" style="width: 64px; height: 64px;" class="q-icon-fallback" />
          <view class="text-grey-6 q-mt-md">请选择或创建一个会话</view>
        </view>
      </view>
    </view>
