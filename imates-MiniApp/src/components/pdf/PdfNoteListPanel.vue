<template>
  <view class="note-panel-container">
    <view class="note-panel-scroll-wrapper">
      <!-- 列表滚动区域：使用 RubberBandList 实现橡皮筋滚动 -->
      <RubberBandList>
        <view class="note-panel-scroll-content">
          <view v-if="notes.length === 0" class="text-grey-5 q-pa-md">
            暂无笔记
          </view>
          <view v-else class="note-list">
            ...
            >
              <!-- 左侧头像：姓名首字 -->
              <view class="note-item-avatar-section">
                <view class="note-avatar">
                  {{ getAvatarLetter(note) }}
                </view>
              </view>
              <view class="note-item-main">
                <view class="note-item-meta">
                  <text class="note-item-author">{{ getAuthorName(note) }}</text>
                  <text class="note-item-separator"> - </text>
                  <text class="note-item-time">{{ formatNoteTime(note) }}</text>
                </view>
                <view class="note-item-content">
                  {{ note.text }}
                </view>
              </view>
              <view class="note-item-side">
                <BaseButton
                  variant="ghost"
                  size="sm"
                  @click.stop="handleDelete(note)"
                >
                  <image src="/icons/delete.svg" style="width: 20px; height: 20px;" />
                </BaseButton>
              </view>
            </view>
          </view>
