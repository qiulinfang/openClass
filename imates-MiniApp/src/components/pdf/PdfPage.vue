<template>
  <view class="pdf-reader-container">
    <!-- 视口区域 -->
    <view
      class="viewport"
      :class="{
    ...
    >
      ...
      >
        ...
        >
          <!-- PDF 内容层 -->
          <canvas type="2d" :ref="(el) => setPdfCanvasRef(el, page.pageIndex)"></canvas>
          <!-- 涂鸦/形状层 -->
          <canvas type="2d" :ref="(el) => setInkCanvasRef(el, page.pageIndex)" class="ink-canvas"></canvas>
        </view>
      ...
      <!-- 加载状态提示 -->
      <view v-if="loading || isRendering" class="loading-overlay">
        <BaseLoading text="正在加载..." :size="48" theme="light" />
      </view>

      <view v-if="readingDirection === 'horizontal'" class="horizontal-nav">
        <BaseButton
          class="nav-btn nav-left"
          variant="ghost"
          size="sm"
          :disabled="isHorizontalFirstPage"
...
const onPointerDown = (e: any) => {
  ...
  activePointers.set(0, { x: clientX, y: clientY })
  stopInertia()

  if (activePointers.size === 1) {
    lastPointerPos = { x: clientX, y: clientY }
    const loc = getPdfPoint(clientX, clientY)
...
const onPointerMove = (e: any) => {
  ...
  if (currentMode.value === 'screenshot' && screenshotStartPoint.value) {
    handleScreenshotDrag(clientX, clientY)
    return
  }
  ...
  if (activePointers.size === 1) {
    ...
    const pts = Array.from(e.touches).map(t => ({ x: (t as any).clientX, y: (t as any).clientY }))
    if (pts.length >= 2) {
      activePointers.set(0, pts[0])
      activePointers.set(1, pts[1])
      handlePinch()
    }
  }
}
