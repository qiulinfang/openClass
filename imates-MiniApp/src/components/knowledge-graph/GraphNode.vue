<template>
  <view class="node-wrapper" :class="wrapperClasses" :style="[wrapperStyle, nodeStyleVariables]">
    <!-- 学习标签 -->
    <view 
    ...
    >
      <!-- 背景图标 -->
      <view class="node-icon" :style="{ backgroundImage: `url(${nodeIcon})` }"></view>
      
      <!-- 中心节点内容在节点内部 -->
      <view v-if="type === 'center'" :class="contentClasses">
        <view class="node-title">{{ formatNodeTitle(node) }}</view>
        <view class="node-chapter">{{ formatNodeChapter(node) }}</view>
      </view>
      
    </view>
    
    <!-- 非中心节点的内容通过绝对定位脱离文档流 -->
    <view v-if="type === 'circular'" :class="contentClasses">
      <view class="node-title">{{ formatNodeName(node) }}</view>
    </view>
    ...
    <view 
      ...
      :ref="(el) => { menuRef = el as any }"
      @click.stop
    >
      <view class="bubble-menu-container">
        <button 
