<template>
  <view class="question-list" @click.capture="handleLinkClick" @click.stop>
    <!-- 全局加载遮罩 -->
    <view v-if="loading || renderingQuestions" class="question-list-loading-overlay">
      <view class="question-list-loading-spinner"></view>
      <view class="question-list-loading-text">题目加载中...</view>
    </view>
    <!-- 搜索输入框 -->
    <view class="search-container">
      ...
      >
        <image :src="searchQuestionIcon" mode="aspectFit" class="photo-search-icon" />
      </BaseButton>
    ...
    >
      <!-- 空状态：根据策略获取文案 -->
      <view v-if="displayedQuestions.length === 0 && !loading" class="native-empty-state">
        <image src="/icons/quiz.svg" style="width: 80px; height: 80px;" class="q-icon-fallback" />
        <view class="text-h6 q-mt-md text-grey-7 native-text-3xl">
          {{
      ...
      <!-- 题目列表 -->
      <view v-if="displayedQuestions.length > 0" class="question-cards-list">
        <view
            ...
            <view class="question-block">
              ...
              <view class="question-header">
                ...
                <view class="question-title-row">
                  ...
                  <!-- 往日错题标记 -->
                  <view v-if="props.showMistakeBadge !== false && mistakeStatus.get(getQuestionUniqueId(question))" class="mistake-badge">
                    <text>往日错题</text>
                  </view>
                  <!-- 题目状态插槽 -->
                ...
                <view class="question-actions">
                  ...
                  >
                    ...
                    >
                      <image :src="askXuebanIcon" mode="aspectFit" class="action-icon" />
                    </BaseButton>
                    ...
                    >
                      <image :src="weikeIcon" mode="aspectFit" class="action-icon" />
                    </BaseButton>
                    ...
                    >
                      <image :src="daanIcon" mode="aspectFit" class="action-icon" :class="{ 'icon-disabled': !canViewAnswer }" />
                    </BaseButton>
                    ...
                    >
                      <image :src="juyifansanIcon" mode="aspectFit" class="action-icon" />
                    </BaseButton>
              ...
              <!-- 题目内容 -->
              <view class="question-content-area">
                <rich-text
                  class="markdown-content question-content"
                  :nodes="renderMessageContent(question?.question || question?.title || '暂无内容')"
                ></rich-text>
              </view>
            </view>
          </view>
      ...
      <!-- 底部分页器 -->
      <view v-if="displayList.length > 0 && totalPages > 1" class="question-pagination">
    ...
    >
      <view class="delete-dialog-options">
        <view class="delete-dialog-toggle-item">
          <text class="delete-dialog-label">同时删除该题目的对话记录</text>
          <switch
        ...
        <view class="delete-dialog-toggle-item">
          <text class="delete-dialog-label">同时删除该题目的草稿</text>
          <switch
            class="delete-dialog-switch"
...
import AnswerView from '../exercise/AnswerView.vue'
import SimilarQuestionList from '../exercise/SimilarQuestionList.vue'
...
// 策略模式支持
import type { QuestionListType } from './strategies'
