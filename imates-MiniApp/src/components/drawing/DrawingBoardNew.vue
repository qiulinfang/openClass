<template>
  <view class="sketchpad-wrapper">
    ...
    >
      <text>确定要清空画板上的所有内容吗？此操作不可撤销。</text>
    </Dialog>
    ...
    >
      ...
      <canvas type="2d" canvas-id="historyCanvas" ref="historyCanvasRef" class="canvas-layer canvas-history"></canvas>
      ...
      <textarea
        ...
        @blur="finishInput"
        @input="(e) => inputState.text = e.detail.value"
      ></textarea>
    ...
    <view class="toast" :class="{ show: showToast }"><text>已导出</text></view>
    ...
    <!-- 缩放控制浮动框 -->
    <view v-if="showZoomControls" class="zoom-control-panel">
      ...
      >
        <template #label> <text>{{ Math.round(camera.zoom * 100) }}%</text> </template>
      </CommonSelect>
      ...
      <button class="zoom-btn zoom-fit" @click="fitToContent">
        <text>F</text>
      </button>
    </view>
...
// --- 引用与状态 ---
const historyCanvasRef = ref<any>(null)
