<template>
  <view class="canvas-demo-container" :class="{ 'toolbar-left': props.toolbarPosition === 'left' }">
    <!-- 统一工具栏（根据 toolbarPosition 浮动在顶部或左侧） -->
    <view class="toolbar-wrapper" :class="{ 'toolbar-wrapper-left': props.toolbarPosition === 'left' }">
      <Toolbar
    ...
    >
      <canvas type="2d" canvas-id="historyCanvas" ref="historyCanvasRef" class="canvas-container canvas-history" :style="canvasStyle"></canvas>
      <canvas type="2d" canvas-id="liveCanvas" ref="liveCanvasRef" class="canvas-container canvas-live" :style="canvasStyle"></canvas>
      ...
      >
        ...
        <view class="zoom-display">{{ Math.round(zoomLevel * 100) }}%</view>
    ...
    <!-- 底部插槽（用于白板页控制等） -->
    <view class="toolbar-bottom-wrapper">
...
import type { ExerciseItem } from '@/types/index.js'
