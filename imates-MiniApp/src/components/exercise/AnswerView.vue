...
<template>
  <view class="answer-view">
    <scroll-view class="full-height" scroll-y>
      <view class="q-pa-md">
        <!-- 答案内容 -->
        <view class="answer-card q-mb-md">
          <view class="answer-card-section">
            <view class="text-subtitle2 text-primary q-mb-sm">
              标准答案
            </view>
            <rich-text
              class="answer-content"
              :nodes="formattedAnswer"
            ></rich-text>
          </view>
        </view>

        <!-- 空状态：只根据答案是否存在判断 -->
        <view v-if="!answer" class="native-empty-state text-center q-pa-xl">
          <view class="text-h6 q-mt-md text-grey-7 native-text-3xl">暂无答案内容</view>
          <view class="text-body2 text-grey-6 q-mt-sm native-text-md">请先选择一道题目</view>
        </view>
      </view>
    </scroll-view>
