...
/**
 * 获取当前用户的 AI 作业会话列表 localforage 实例
 */
function getHomeworkSessionsLocalForage() {
  const userId = getUserId()
  return localforage.createInstance({
    driver: localforage.INDEXEDDB,
    name: `ExerciseSolveApp_${userId}`,
    version: 1.0,
    storeName: 'ai_homework_sessions',
    description: `AI作业会话列表存储 (用户: ${userId})`
  })
}
...
export class ChatStorageService {
  ...
  async saveHomeworkSessions(sessions: any[]): Promise<void> {
    try {
      await this.initialize()
      const homeworkSessionsForage = getHomeworkSessionsLocalForage()
      const key = 'ai_homework_sessions'
      const plainSessions = JSON.parse(JSON.stringify(sessions))
      await homeworkSessionsForage.setItem(key, plainSessions)
    } catch (error) {
      console.error('[CHAT_STORAGE] 保存 AI 作业会话列表失败:', error)
      throw error
    }
  }
  ...
  async loadHomeworkSessions(): Promise<any[]> {
    try {
      ...
      const homeworkSessionsForage = getHomeworkSessionsLocalForage()
      const key = 'ai_homework_sessions'
      const stored = await homeworkSessionsForage.getItem<any[]>(key)
      return stored || []
    } catch (error) {
