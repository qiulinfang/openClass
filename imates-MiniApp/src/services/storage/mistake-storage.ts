 ...
 * 获取错题存储专用的存储服务实例
 */
function getMistakeStorage(): IndexedDBService | MiniAppStorageService {
  ...
  // 小程序环境使用 MiniAppStorageService
  // @ts-ignore
  if (typeof uni !== 'undefined' && uni.getStorage) {
    return MiniAppStorageService.getInstance(config)
  }

  return IndexedDBService.getInstance(config as any)
}
