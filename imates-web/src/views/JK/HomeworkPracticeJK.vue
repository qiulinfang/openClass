...
const handleSingleQuestionSubmit = async () => {
  ...
  if (currentAnswerQuestion.value.type === 'single_choice' || currentAnswerQuestion.value.type === 'multiple_choice') {
    userAnswer = cache.chooseList || []
    const correctAnswer = currentAnswerQuestion.value.answer
    if (Array.isArray(correctAnswer)) {
      isCorrect = userAnswer.length === correctAnswer.length && 
                  userAnswer.every((v: string) => correctAnswer.includes(v))
    } else {
