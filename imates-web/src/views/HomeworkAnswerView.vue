...
const getQuestionStatus = (question: ExerciseItem): QuestionStatus => {
  ...
  if (type === 'single_choice' || type === 'multiple_choice') {
    ...
    hasUserAnswer =
      Array.isArray(val) &&
      val.some((v: any) => {
        return v && (v.boardData || v.photoUrl)
      })
  } else if (type === 'composite') {
    hasUserAnswer = val && typeof val === 'object' && Object.keys(val).length > 0
  } else if (type === 'subjective') {
    hasUserAnswer = val && (hasBoardAnswerData(val.boardData) || val.photoUrl)
  }
    ...
    if (Array.isArray(userAnswer)) {
      answers = userAnswer.map((item: any) => {
        if (typeof item === 'object' && item !== null) {
          const itemType = item.type || 'board'
          let content: any = ''
          if (itemType === 'board') {
            content = {
              boardData: item.boardData || null,
              boardImg: item.boardImg || null,
            }
        ...
        return { type: 'board', content: null }
      })
    } else {
      answers = []
    }
    ...
    if (typeof item === 'object' && item !== null) {
      ...
      if (valStr.startsWith('{') && valStr.includes('"objects"')) {
        try {
          ...
          answers = { type: 'board', content: null }
        }
      } else {
        answers = { type: 'board', content: null }
      }
...
// 递归函数：将作答内容中的所有 base64 图片/手写板数据上传到云端，并规范类型为 text / img 后提交给后端
const uploadAnswersImages = async (questionAnswerList: any[]) => {
  for (const qAns of questionAnswerList) {
    // 1. 处理主观题/填空题等具体作答中的 base64，并将 board 和 photo 转换规范为 img
    if (qAns.type === 'subjective') {
      if (qAns.answers && typeof qAns.answers === 'object') {
        const ans = qAns.answers
        if (ans.type === 'photo' || ans.type === 'img') {
          if (typeof ans.content === 'string' && ans.content.startsWith('data:image')) {
            console.log('[HomeworkSubmit] 上传主观题照片到云端...')
            const path = await apiService.uploadImageToYanban(ans.content)
            ans.content = path
          }
          ans.type = 'img' // 强制转为 img 类型传给后端
        } else if (ans.type === 'board' && ans.content && typeof ans.content === 'object') {
          const boardImg = ans.content.boardImg
          if (typeof boardImg === 'string' && boardImg.startsWith('data:image')) {
            console.log('[HomeworkSubmit] 上传主观题手写板截图到云端...')
            const path = await apiService.uploadImageToYanban(boardImg)
            ans.content = path
          } else {
            ans.content = ''
          }
          ans.type = 'img' // 将手写板转为 img 类型传给后端
        }
      }
      ...
      if (Array.isArray(qAns.answers)) {
        for (const ans of qAns.answers) {
          if (ans.type === 'photo' || ans.type === 'img') {
            if (typeof ans.content === 'string' && ans.content.startsWith('data:image')) {
              console.log('[HomeworkSubmit] 上传填空题照片到云端...')
              const path = await apiService.uploadImageToYanban(ans.content)
              ans.content = path
            }
            ans.type = 'img' // 强制转为 img 类型传给后端
            ...
            const boardImg = ans.content.boardImg
            if (typeof boardImg === 'string' && boardImg.startsWith('data:image')) {
              console.log('[HomeworkSubmit] 上传填空题手写板截图到云端...')
              const path = await apiService.uploadImageToYanban(boardImg)
              ans.content = path
            } else {
              ans.content = ''
