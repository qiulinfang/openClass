...
const saveCurrentPage = async () => {
  ...
  // 2. 如果没有白板或白板没数据，但有结构化题目渲染区域，则截图该区域作为答案图片
  if (!imageData && currentQuestionRenderRef.value) {
    try {
      // 截图当前的题目交互区域（包含学生的选项/填空）
      const dataUrl = await htmlToImage.toPng(currentQuestionRenderRef.value, {
      ...
      console.error('[HomeworkAnswerView] 截图题目作答区域失败:', e)
    }
  }
