            ...
            <div class="question-block">
              ...
              <div class="card-header">
                ...
                <div class="question-title-row">
                  <div class="question-no">探究{{ getQuestionDisplayIndex(getQuestionUniqueId(question)) }}</div>
                  <!-- 题目状态插槽 -->
                  <slot name="question-status" :question="question" :index="index" />
                </div>

                <!-- 右侧：功能区（仅当前题目选中时显示） -->
                <div class="question-actions" v-if="isQuestionSelected(getQuestionUniqueId(question))">
                  <CommonActionButton
                    label="开始探究"
                    variant="primary"
                    size="sm"
...
const sendToAi = async (question: ExerciseItem) => {
  try {
    ...
    // 如果父组件是核心探究 (JK版)，使用 aiTextbookChatStore
    if (props.showQuestionActions === true) {
      const aiTextbookStore = useAiTextbookChatStore()
      
      // 设置资源ID (使用题目ID)
      aiTextbookStore.setResourceId(question.bmNo)
      ...
      aiTextbookStore.setChapterInfo({
        ...
        subject: aiSubject,
        textbook: '核心探究',
        chapter_title: question.title || '核心探究'
      })

      // 清除历史并开始新对话
      await aiTextbookStore.clearChatHistory()

      const questionContent = question.question || question.title || '题目内容为空'
