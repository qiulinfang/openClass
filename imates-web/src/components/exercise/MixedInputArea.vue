...
const initFromValue = (val: MixedModelValue | undefined) => {
  ...
  if (props.questionType === 'fill') {
    ...
    if (valStr.startsWith('{') && valStr.includes('"type":"photo"')) {
...
const handleBoardSave = (boardData: any) => {
  const boardImg = drawingBoardRef.value?.exportToJpg?.(0.9) || null
  if (props.questionType === 'fill') {
    const dataObj = typeof boardData === 'object' && boardData !== null ? boardData : {}
    const str = JSON.stringify({ ...dataObj, boardImg })
