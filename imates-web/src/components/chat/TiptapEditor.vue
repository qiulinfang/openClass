  ...
  addNodeView() {
    return ({ node, getPos, editor }) => {
      ...
      const initMathField = async () => {
        ...
        // 使用统一的事件监听器管理函数
        console.log('🧮 [FORMULA-MATHLIVE] 设置统一事件监听器')
        // 设置防抖输入处理
        let inputTimeout: number
        const debouncedInputHandler = (value: string) => {
          console.log('🧮 [FORMULA-EDIT] 公式内容变化:', value || '(空)')
          ...
          inputTimeout = window.setTimeout(() => {
            console.log('⏰ [FORMULA-EDIT] 防抖超时，更新公式内容')
            ...
            // 触发内容更新事件，通知 ChatInput
            if (typeof window !== 'undefined' && window.dispatchEvent) {
              const updateEvent = new CustomEvent('formula-content-updated', {
                detail: {
        ...
        setupUnifiedMathFieldListeners(mathField, {
          onInput: debouncedInputHandler,
          onVirtualKeyboardToggle: () => {
            // 虚拟键盘切换的额外处理逻辑可以在这里添加
          },
        })
 ...
 */
const setupUnifiedMathFieldListeners = (
  mathField: any,
  options: {
    ...
    onVirtualKeyboardToggle?: (event: Event) => void
  },
) => {
  console.log('🧮 [FORMULA-EVENTS] 开始设置统一的 MathLive 事件监听器')

  // 清理现有监听器（如果存在）
  if (mathField._eventListeners) {
    Object.entries(mathField._eventListeners).forEach(([event, handler]) => {
      mathField.removeEventListener(event, handler)
    })
  }

  // 存储监听器引用以便后续清理
  mathField._eventListeners = {}

  // 设置虚拟键盘切换事件监听器
  if (options.onVirtualKeyboardToggle) {
    const keyboardToggleHandler = (event: Event) => {
      ...
      // 调用自定义处理函数
      if (options.onVirtualKeyboardToggle) {
        options.onVirtualKeyboardToggle(event)
      }
    }

    mathField.addEventListener('virtual-keyboard-toggle', keyboardToggleHandler)
    mathField._eventListeners['virtual-keyboard-toggle'] = keyboardToggleHandler
  }
  ...
  if (options.onInput) {
    const inputHandler = () => {
      ...
      console.log('🧮 [FORMULA-INPUT] 公式内容变化:', value || '(空)')
      options.onInput!(value)
    }

    mathField.addEventListener('input', inputHandler)
    mathField._eventListeners['input'] = inputHandler
  }
  ...
  if (options.onFocus) {
    ...
    mathField._eventListeners['focus'] = focusHandler
  }
  ...
  if (options.onBlur) {
    ...
    mathField.addEventListener('blur', blurHandler)
    mathField._eventListeners['blur'] = blurHandler
  }
  ...
  if (options.onKeydown) {
    ...
    mathField._eventListeners['keydown'] = keydownHandler
  }

  console.log('✅ [FORMULA-EVENTS] 统一事件监听器设置完成')
}
 ...
 * @param mathField MathLive 实例
 */
const cleanupMathFieldListeners = (mathField: any) => {
  if (mathField._eventListeners) {
    console.log('🧹 [FORMULA-EVENTS] 清理 MathLive 事件监听器')
    Object.entries(mathField._eventListeners).forEach(([event, handler]) => {
      mathField.removeEventListener(event, handler)
    })
    delete mathField._eventListeners
    console.log('✅ [FORMULA-EVENTS] 事件监听器清理完成')
  }
}
 ...
 */
const handleInitialFocus = async (
  mathField: any,
...
const setupUserInteractionListeners = (
  ...
  setupUnifiedMathFieldListeners(mathField, {
    ...
    onBlur: () => {
      ...
      const nodeId = container.getAttribute('data-node-id')
      if (nodeId) {
        // 检查公式内容是否为空，如果为空则取消编辑
        const content = mathField.getValue() || ''
        if (!content.trim()) {
          console.log('⚠️ [FORMULA-BLUR] 公式内容为空，取消编辑')
          // 触发取消编辑事件
          if (typeof window !== 'undefined' && window.dispatchEvent) {
            const cancelEvent = new CustomEvent('formula-cancel-edit', {
              detail: { nodeId },
            })
    ...
    onKeydown: (event: KeyboardEvent) => {
      console.log('⌨️ [FORMULA-KEYBOARD] 检测到键盘事件', {
        key: event.key,
      ...
      if (event.key === 'Enter' && !event.shiftKey) {
        console.log('📤 [FORMULA-KEYBOARD] 检测到回车键，完成编辑')
        event.preventDefault()
        deactivate()

        // 如果是 ChatInput 环境，触发发送消息
        if (typeof window !== 'undefined' && window.dispatchEvent) {
          const sendMessageEvent = new CustomEvent('formula-enter-pressed')
          window.dispatchEvent(sendMessageEvent)
        }
      } else if (event.key === 'Escape') {
        console.log('🚫 [FORMULA-KEYBOARD] 检测到ESC键，取消编辑')
        event.preventDefault()
        deactivate()
      }
    },
...
const handleDocumentClick = (event: MouseEvent) => {
  ...
  const isClickInsideFormula =
    target.closest('.formula-node-container') ||
  ...
  const isClickOnKeyboardOverlay =
    target.classList.contains('ML__keyboard-overlay') ||
    target.classList.contains('ML__keyboard-backdrop')

  // 检查是否点击在输入框或其他交互元素上（这些不应该触发公式失活）
  const isClickOnInputElement =
...
const deactivateAllFormulas = () => {
  ...
  console.log(
    '🧮 [FORMULA-DEACTIVATE-ALL] 开始失活所有公式节点，数量:',
    activeFormulaContainers.length,
  )
  ...
  activeFormulaContainers.forEach((container: Element, index: number) => {
    ...
    if (mathField) {
      try {
        ...
        // 1. 清理事件监听器
        cleanupMathFieldListeners(mathField)
...
defineExpose({
  ...
  hideAllVirtualKeyboards, // 暴露隐藏所有虚拟键盘的方法
  deactivateAllFormulas, // 暴露失活所有公式的方法
})
...
.editor-content {
  ...
  background: transparent;
  font-family:
    -apple-system, BlinkMacSystemFont, 'Helvetica Neue', 'PingFang SC', 'Hiragino Sans GB',
