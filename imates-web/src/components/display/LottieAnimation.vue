<template>
  <div class="lottie-animation-container">
    ...
    <!-- 图片序列动画模式 -->
    <div v-else-if="imageSequencePath" class="image-sequence-player">
      <img
        ref="animationImage"
        :src="currentFrameSrc"
        :style="imageStyle"
      ...
      <div v-if="showControls" class="animation-controls">
        ...
        <input
          ...
          class="frame-slider"
        />
        <span class="frame-info">{{ currentFrame }}/{{ frameCount - 1 }}</span>
...
const preloadNextFrames = (startFrame: number) => {
  for (let i = 0; i < props.preloadFrames; i++) {
    const frameToPreload = startFrame + i
    if (frameToPreload <= props.endFrame && !preloadedImages.value.has(frameToPreload)) {
      const img = new Image()
      img.src = `${props.imageSequencePath}12_${frameToPreload}.jpg`
      preloadedImages.value.set(frameToPreload, img)
    }
  }
}
...
const preloadAllFrames = async () => {
  ...
  for (let frame = props.startFrame; frame <= props.endFrame; frame++) {
    if (!preloadedImages.value.has(frame)) {
      promises.push(new Promise((resolve, reject) => {
        const img = new Image()
        img.onload = () => {
          preloadedImages.value.set(frame, img)
          resolve()
        }
        img.onerror = reject
        img.src = `${props.imageSequencePath}12_${frame}.jpg`
      }))
    }
  }
