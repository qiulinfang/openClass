...
static int bufferStreamNext(fz_context *ctx, fz_stream *stream, int max)
{
	...
	(*env)->GetByteArrayRegion(env, array, stream->pos, len, (jbyte *)bs->buffer);
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_addInkAnnotationInternal)(JNIEnv * env, jobject thiz, jobjectArray arcs, float colorR, float colorG, float colorB, float inkThickness)
{
	globals *glo = get_globals(env, thiz);
	fz_context *ctx = glo->ctx;
	fz_document *doc = glo->doc;
	...
	fz_try(ctx)
	{
		...
		pdf_set_ink_annot_list(ctx, idoc, (pdf_annot *)annot, pts, counts, n, color, inkThickness);//INK_THICKNESS);
...
JNI_FN(MuPDFCore_hitAnnotation)(JNIEnv *env, jobject thiz, jint page, jfloat x, jfloat y)
{
    globals *glo = get_globals(env, thiz);
    if (!glo) return -1;
    fz_context *ctx = glo->ctx;
    ...
    fz_try(ctx)
    {
        ...
        // 遍历页面上的所有 annotations
        annot = pdf_first_annot(ctx, page_obj);
        annot_index = 0;
        
        while (annot)
        {
            // 检查 annotation 类型是否为 ink
            if (pdf_annot_type(ctx, annot) == FZ_ANNOT_INK)
            {
                // 获取 annotation 的边界框
                fz_rect rect;
				pdf_bound_annot(ctx, annot, &rect);
                
                // 检查点是否在边界框内
                if (x >= rect.x0 && x <= rect.x1 && y >= rect.y0 && y <= rect.y1)
                {
                    // 更详细的检查：点是否靠近 ink 路径
                    // 这需要更复杂的实现，这里简化为边界框检查
                    fz_drop_page(ctx, (fz_page*)page_obj); // 提前释放
                    return annot_index;
                }
            }
    ...
    fz_catch(ctx)
    {
        LOGE("hitAnnotation: %s", fz_caught_message(ctx));
    }
	if (page_obj)
    	fz_drop_page(ctx, (fz_page*)page_obj);
    
    return -1;
}

JNIEXPORT jobjectArray JNICALL
JNI_FN(MuPDFCore_getAnnotationInkList)(JNIEnv *env, jobject thiz, jint page, jint annot_index)
{
    globals *glo = get_globals(env, thiz);
    if (!glo) return NULL;
    fz_context *ctx = glo->ctx;
    ...
    fz_try(ctx)
    {
        ...
        // 获取指定索引的 annotation
        annot = pdf_first_annot(ctx, page_obj);
        for (int i = 0; i < annot_index && annot; i++)
        {
            annot = pdf_next_annot(ctx, annot);
        }
        
        if (annot && pdf_annot_type(ctx, annot) == FZ_ANNOT_INK)
        {
            // 获取 ink annotation 的路径数据
            pdf_obj *ink_list = pdf_dict_gets(ctx, annot->obj, "InkList");
            int n_paths = ink_list ? pdf_array_len(ctx, ink_list) : 0;
            
            // 创建 Java 数组
            jclass pointFClass = (*env)->FindClass(env, "android/graphics/PointF");
            if (!pointFClass) fz_throw(ctx, FZ_ERROR_GENERIC, "FindClass failed");
            jmethodID pointFConstructor = (*env)->GetMethodID(env, pointFClass, "<init>", "(FF)V");
            if (!pointFConstructor) fz_throw(ctx, FZ_ERROR_GENERIC, "GetMethodID failed");
            
            jclass arrayClass = (*env)->FindClass(env, "[Landroid/graphics/PointF;");
            if (!arrayClass) fz_throw(ctx, FZ_ERROR_GENERIC, "FindClass failed");
            
            result = (*env)->NewObjectArray(env, n_paths, arrayClass, NULL);
            if (!result) fz_throw(ctx, FZ_ERROR_GENERIC, "NewObjectArray failed");
            
            for (int i = 0; i < n_paths; i++)
            {
                pdf_obj *path = pdf_array_get(ctx, ink_list, i);
                int n_points = path ? (pdf_array_len(ctx, path)/2) : 0;
                
                jobjectArray pointArray = (*env)->NewObjectArray(env, n_points, pointFClass, NULL);
                if (!pointArray) fz_throw(ctx, FZ_ERROR_GENERIC, "NewObjectArray failed");
                
                for (int j = 0; j < n_points; j++)
                {
                    float x = pdf_to_real(ctx, pdf_array_get(ctx, path, j*2));
                    float y = pdf_to_real(ctx, pdf_array_get(ctx, path, j*2+1));
                    
                    jobject point = (*env)->NewObject(env, pointFClass, pointFConstructor, x, y);
                    if (!point) fz_throw(ctx, FZ_ERROR_GENERIC, "NewObject failed");
                    (*env)->SetObjectArrayElement(env, pointArray, j, point);
                    (*env)->DeleteLocalRef(env, point);
                }
                
                (*env)->SetObjectArrayElement(env, result, i, pointArray);
                (*env)->DeleteLocalRef(env, pointArray);
            }
        }
    }
    fz_catch(ctx)
    {
        LOGE("getAnnotationInkList: %s", fz_caught_message(ctx));
        result = NULL;
    }
...
JNIEXPORT jboolean JNICALL
JNI_FN(MuPDFCore_updateInkAnnotation)(JNIEnv *env, jobject thiz, jint page, jint annot_index, jobjectArray arcs, jfloatArray colors, jfloat thickness)
{
    globals *glo = get_globals(env, thiz);
    if (!glo) return JNI_FALSE;
    fz_context *ctx = glo->ctx;
    pdf_document *doc = pdf_specifics(ctx, glo->doc);
    pdf_page *page_obj = NULL;
    pdf_annot *annot = NULL;
    ...
    fz_try(ctx)
    {
        ...
        {				
			fz_matrix ctm;
			// 获取颜色
			jfloat *colorArray = (*env)->GetFloatArrayElements(env, colors, NULL);
			color[0] = colorArray[0];
			color[1] = colorArray[1];
			color[2] = colorArray[2];
			(*env)->ReleaseFloatArrayElements(env, colors, colorArray, JNI_ABORT);

            // 创建新的 ink annotation 数据
            float zoom = glo->resolution / 72;
            zoom = 1.0 / zoom;
            fz_scale(&ctm, zoom, zoom);
            
            pt_cls = (*env)->FindClass(env, "android/graphics/PointF");
            if (!pt_cls) fz_throw(ctx, FZ_ERROR_GENERIC, "FindClass failed");
            x_fid = (*env)->GetFieldID(env, pt_cls, "x", "F");
            if (!x_fid) fz_throw(ctx, FZ_ERROR_GENERIC, "GetFieldID(x) failed");
            y_fid = (*env)->GetFieldID(env, pt_cls, "y", "F");
            if (!y_fid) fz_throw(ctx, FZ_ERROR_GENERIC, "GetFieldID(y) failed");

            n = (*env)->GetArrayLength(env, arcs);
            if (n <= 0) fz_throw(ctx, FZ_ERROR_GENERIC, "Empty arcs array");

            counts = fz_malloc_array(ctx, n, sizeof(int));
            for (i = 0; i < n; i++)
            {
                jobjectArray arc = (jobjectArray)(*env)->GetObjectArrayElement(env, arcs, i);
                if (!arc) fz_throw(ctx, FZ_ERROR_GENERIC, "Null arc array");
                counts[i] = (*env)->GetArrayLength(env, arc);
                total += counts[i];
                (*env)->DeleteLocalRef(env, arc);
            }
			...
			pdf_set_ink_annot_list(ctx, doc, annot, pts, counts, n, color, thickness);
    ...
    fz_always(ctx)
    {
        if (pts) fz_free(ctx, pts);
        if (counts) fz_free(ctx, counts);
        if (page_obj) fz_drop_page(ctx, (fz_page*)page_obj);
    }
    fz_catch(ctx)
    {
        LOGE("updateInkAnnotation: %s", fz_caught_message(ctx));
    }
    return result;
}
