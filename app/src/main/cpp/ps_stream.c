...
static int set_sys_header(lt_program_spec *spec, uint8_t *dest, int maxlen, int rate_bound) {
...
static int set_psm(lt_program_spec *spec, uint8_t *dest, int maxlen) {
...
int lts_ps_stream(lt_es_frame *frame, uint8_t *dest, int maxlen, lt_ts_program *pi) {
	...
	int ret = 0;
	int ps_len = 0;
	lt_program_spec *spec = &pi->prog[0];
	const uint8_t *fdata;
	int flen;
	uint64_t pts;
	int PES_LEN, pes_num;
