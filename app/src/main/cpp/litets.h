...
typedef struct
{
	...
	lt_stream_spec stream[MAX_STREAM_NUM];
} lt_program_spec;
...
typedef struct
{
	...
	lt_program_spec prog[MAX_PROGRAM_NUM];
} lt_ts_program;
...
int lts_ts_stream(lt_es_frame *frame, uint8_t *dest, int maxlen, lt_ts_program *pi);

// 返回PS的长度，出错（如dest空间不足）返回-1。 
int lts_ps_stream(lt_es_frame *frame, uint8_t *dest, int maxlen, lt_ts_program *pi);
...
// ts_buf是传入的TS流的缓冲
int lts_ts_demux(lt_demux *handle, uint8_t *ts_buf, int len);

// PS码流解复用，成功返回已处理长度，失败返回-1
// ps_buf是传入的PS流的缓冲
int lts_ps_demux(lt_demux *handle, uint8_t *ps_buf, int len);
...
int lts_buffer_handle(lt_buffer_handler *handler);
