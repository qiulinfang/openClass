...
// 生成一个PAT的TS包
static void gen_pat_ts_packet(char *pkt, lt_ts_program *pi) {
	// TS流ID目前取固定值
...
// 生成一个PMT的TS包
static void gen_pmt_ts_packet(char *pkt, int pno, lt_program_spec *ps) {
...
// 将一帧生成若干TS包, 返回生成的字节数
static int gen_pes_ts_packets(lt_es_frame *frame, char *dest, int maxlen, lt_ts_program *pi) {
	...
	int total_length = 0;
	lt_stream_spec *ss = &pi->prog[frame->program_number].stream[frame->stream_number];
