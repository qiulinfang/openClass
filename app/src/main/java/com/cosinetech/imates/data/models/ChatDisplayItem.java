...
public class ChatDisplayItem {
    ...
    private boolean isTypingAnimationActive = false;
    private final Context context;

    public boolean initialDisplayed = false;

    private final IncrementalStringProcessor processor = new  IncrementalStringProcessor();
    ...
    public ChatDisplayItem(ChatMessage msg, boolean showWithTypingEffect, Context context) {
        ...
        this.msgContentIsFinished = !showWithTypingEffect;
    }
...
//                        });
    ...
    public void startTypingEffect(TypingEffectCallback callback) {
        ...
        typingRunnable = new Runnable() {
            @Override
            public void run() {
                if (currentDisplayCharIndex < chatMessage.content.length() && isTypingAnimationActive) {
                ...
                if (!msgContentIsFinished || currentDisplayCharIndex < chatMessage.content.length()) {
                    // 计算下一个字符的延迟时间
                    long delay = 100;
