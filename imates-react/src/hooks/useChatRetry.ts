...
export function useChatRetry(options: UseChatRetryOptions = {}) {
  ...
  const prepareRetryInfo = useCallback((messages: ChatBubble[], messageId: string): RetryInfo => {
  ...
  const retryByDeleteAndResend = useCallback(async (params: {
    ctx: RetryContext
    messageId: string
    resend: (payload: {
      originalContent: string
    ...
    const index = ctx.messages.findIndex((m) => m.id === messageId)
    if (index < 0) {
      throw new Error('消息不存在')
    }
