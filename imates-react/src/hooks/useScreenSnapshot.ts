...
export function useScreenSnapshot() {
  const [isSnapshotPreparing, setIsSnapshotPreparing] = useState(false)
  
  const isAndroidSnapshotAvailable = useMemo(() => {
    return androidBridge.isAndroidBridgeAvailable() && typeof window !== 'undefined' && !!(window as any).AndroidBridge?.takeSnapshot
  }, [])

  const captureScreenSnapshot = useCallback(async (): Promise<ScreenSnapshotResult> => {
    setIsSnapshotPreparing(true)

    try {
      await new Promise((r) => requestAnimationFrame(() => r(null)))

      if (isAndroidSnapshotAvailable) {
        return await captureFromAndroid()
      }

      return await captureFromWeb()
    } finally {
      setIsSnapshotPreparing(false)
    }
