...
export class UserClientStrategy implements ChatStrategy {
  ...
  getWelcomeMessage(): string { return '连接中...' }
  requiresQuestion(): boolean { return false }
  getMessageType(): Sender { return Sender.USER }
  getSenderType(): Sender { return Sender.USER }
  ...
  async forwardMessages(messages: ChatBubble[], options: ForwardOptions = {}): Promise<ForwardResult> { return { success: false, error: '客户端对话不支持转发' } }
