...
export function validateKnowledgeTopicAndAck2Request(body: unknown): void {
  ...
  if (bmNoList === undefined || bmNoList === null || (typeof bmNoList !== 'string' && !Array.isArray(bmNoList))) {
    throw new Error('[knowledgeTopicAndAck2] bmNoList 不能为空')
  }

  let items: string[] = []
  if (Array.isArray(bmNoList)) {
    items = bmNoList.map(s => String(s).trim()).filter(s => s.length > 0)
  } else if (typeof bmNoList === 'string') {
    items = bmNoList
      .split(',')
      .map((s: string) => s.trim())
