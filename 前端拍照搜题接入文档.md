...
`/v2/ocr` 会调用付费 OCR 服务，需要携带 `X-OCR-API-Key`。

生产环境不要把该密钥硬编码进网页、JavaScript 包、APP 安装包或 Git 仓库，否则任何人都可以提取密钥并消耗 OCR 额度。

当前测试服务是 HTTP 地址。如果业务页面使用 HTTPS，浏览器会拦截直接访问 HTTP
接口。生产环境必须由业务后端代理，或者为 MathRAG 配置 HTTPS 域名。
...
如果是受控内网联调，可以临时由前端配置接口密钥，但不能提交到代码仓库。密钥由后端负责人通过安全渠道提供。
...
```typescript
const API_BASE_URL = "http://49.232.39.212:18211";
...
export async function recognizeStudentPhoto(
  ...
  const response = await fetch(`${API_BASE_URL}/v2/ocr`, {
    method: "POST",
    headers: {
...
export async function searchSelectedQuestion(
  ...
  const response = await fetch(`${API_BASE_URL}/v2/search_image`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
业务后端携带服务端保存的 OCR 密钥调用：

```bash
curl -X POST 'http://49.232.39.212:18211/v2/ocr' \
  -H 'X-OCR-API-Key: <OCR 接口密钥>' \
  -F 'image=@student.jpg;type=image/jpeg'
```
